...
def start_over_n_uniform(low, high, constraints=None, real=None, error=None, **kwargs):
...
def mean_normal(low, high, constraints=None, real=None, error=None, **kwargs):
    ...
    mu = 0.621457 
    sigma = 0.049797073
    # create a truncated norm for each logit and take the mean
    means = np.array([truncnorm((low[i] - mu) / sigma, (high[i] - mu) / sigma, loc=mu, scale=sigma).mean() for i in range(len(low))])
    # check whether the means are in between low and high, just print for which it holds or not
    in_range = [(low[i] <= means[i] <= high[i]) for i in range(NTOK)]
    for i, valid in enumerate(in_range):
        if not valid:
            print(f"Mean {means[i]:.4f} out of bounds [{low[i]:.4f}, {high[i]:.4f}] for token {i}")
    r = 1 - means
    r[base_top_token] = 0
    # plot mid and use low and high as error bars
    if kwargs.get('plot', False):        
        ...
        plt.figure(figsize=(15, 8))  # Set the figure size to full screen
        plt.errorbar(range(len(srt_mid)), srt_mid, yerr=[srt_mid - srt_low, srt_high - srt_mid], fmt='o', capsize=4, color='blue', ecolor='black', capthick=2, elinewidth=2)
        plt.fill_between(range(NTOK), [1 - srt_r[i] for i in range(NTOK)], [1 for _ in range(NTOK)], color='lightcoral', label='Bias')        
        plt.scatter(range(NTOK), srt_mid, color='blue', label='Mid', marker='o')
        real = np.sort(real)
        plt.scatter(range(len(real)), real, color='yellow', label='Real', marker='^')
        plt.title(f"Distribution with Area Error: {error}")
...
def start_one_over_n_normal(low, high, constraints=None, real=None, error=None, **kwargs):
    ...
    if kwargs.get('plot', True) and 98 <= len(constraints) <= 101:        
        ...
        srt_high = high[order]
        srt_mid = mid[order]
        mid = (srt_high + srt_low) / 2
        radius = (srt_high - srt_low) / 2
        weighted_guess = mid + ((1 - srt_high) / (srt_low + (1 - srt_high))) * radius - (srt_low / (srt_low + (1 - srt_high))) * radius
        srt_r = r[order]
        print(srt_low.shape, srt_high.shape, srt_mid.shape)
        plt.figure(figsize=(15, 8))  # Set the figure size to full screen
        plt.errorbar(range(len(srt_mid)), srt_mid, yerr=[srt_mid - srt_low, srt_high - srt_mid], fmt='o', capsize=4, color='blue', ecolor='black', capthick=2, elinewidth=2)
        plt.fill_between(range(NTOK), [1 - srt_r[i] for i in range(NTOK)], [1 for _ in range(NTOK)], color='lightcoral', label='Bias')        
        plt.scatter(range(NTOK), srt_mid, color='blue', label='Guess: $\\mathbf{g}_m$', marker='o')
        plt.scatter(range(NTOK), weighted_guess, color='red', label='Guess: $\\mathbf{g}_w$', marker='x', s=30, zorder=5)
        real = np.sort(real)
        plt.scatter(range(len(real)), real, color='yellow', label='Real', marker='^', s=80, zorder=4)
