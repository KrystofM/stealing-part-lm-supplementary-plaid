...
def test_calculute_products():
    ...
    simple_r_values = [0, 0]
    simple_i = 0
    simple_product = calculate_product(simple_intervals[0][0], simple_intervals[0][1], simple_r_values[0], simple_intervals, simple_mu, simple_sigma, simple_r_values, simple_i)
    print('Simple product:', simple_product)
    simple_product_d = calculate_product_d(simple_intervals[0][0], simple_intervals[0][1], simple_r_values[0], simple_intervals, simple_mu, simple_sigma, simple_r_values, simple_i)
    print('Simple product d:', simple_product_d)
    ...
    print('Simple product d vectorized:', simple_product_d_vectorized)
    new_r = [0.5, 0]
    simple_product_d_vectorized = calculate_product_d_vectorized(simple_intervals[1][0], simple_intervals[1][1], simple_r_values[1], simple_intervals, simple_mu, simple_sigma, new_r, 1)
    print('Simple product d vectorized:', simple_product_d_vectorized)
    simple_product_d_vectorized = calculate_product_d_vectorized(simple_intervals[0][0], simple_intervals[0][1], simple_r_values[0], simple_intervals, simple_mu, simple_sigma, new_r, 0)
    print('Simple product d vectorized:', simple_product_d_vectorized)

test_calculute_products()
...
def system_of_equations(r_values, intervals, mu, sigma, n):
    ...
    for i in range(1, len(intervals)):
        l_i, h_i = intervals[i]
        product = calculate_product(l_i, h_i, r_values[i], intervals, mu, sigma, r_values, i)
...
intervals = [(0.1, 0.5), (0.3, 0.6), (0.2, 0.7), (0.0, 0.4), (0.2, 0.8), (0.99, 1.0)]
N = len(intervals)
r_values = []
...
# Solve for r_values
r_values = least_squares(system_of_equations, initial_guess, args=(intervals, mean, std, N)).x
