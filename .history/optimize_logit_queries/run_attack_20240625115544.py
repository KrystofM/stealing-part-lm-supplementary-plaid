...
error_funcs = {
    ...
    "l1_bounds": lambda low, high, _: np.sum(high - low),
    "weird": lambda low, high, real: np.sum(np.minimum(high - real, real - low)),
    "weighted_guess": lambda low, high, real: mitka_error(low, high, real),
}
...
query_strategies = {
    ...
    "every_one_over_n_uniform": every_one_over_n_uniform,
...
def run_attack(
    ...
    **kwargs,
    ...
    steps = 0
    end_steps = None
    frequency_bins_over_time = []
    while steps < MAXQ:
        bias = query_strategy_func(
            low, high, constraints=constraints, bounds_data=bounds_data, real=real, error=current_error, **kwargs
        )
        print(f"Bias: {bias}")
        print(f"Bias + real: {real + bias}")
        potential_top_tokens = np.flatnonzero((real + bias) == np.max(real + bias))
        top_token = np.random.choice(potential_top_tokens)
        ...
        frequency_bins, _ = np.histogram([x[0] for x in constraints], bins=NTOK, range=(0, NTOK))
        frequency_bins_over_time.append(frequency_bins)
    ...
    # plt.imshow(frequency_bins_over_time, aspect='auto', interpolation='nearest')
    # plt.colorbar(label='Frequency')
    # plt.xlabel('Token Index')
    # plt.ylabel('Step')
    # plt.title(f"Change of Sampling Frequency Over Time for {label}")
