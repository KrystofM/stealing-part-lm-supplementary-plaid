...
def start_one_over_n_normal(low, high, constraints=None, real=None, error=None, **kwargs):
    ...
    if kwargs.get('plot', True) and 98 <= len(constraints) <= 101:        
        ...
        plt.scatter(range(NTOK), weighted_guess, color='red', label='Guess: $\\mathbf{g}_w$', marker='x', s=10, zorder=5)
        real = np.sort(real)
        plt.scatter(range(len(real)), real, color='yellow', label='Real', marker='^', s=30)
