...
# Vectorized version
def calculate_product_d_vectorized(l_i, h_i, intervals, mu, sigma, i):
    tnormI = truncnorm((l_i - mu) / sigma, (h_i - mu) / sigma, loc=mu, scale=sigma)    
    ...
    for j, (l_j, h_j) in enumerate(intervals):
        if j != i:
            ...
            cdf_values = tnormJ.cdf(y_values)
...
def test_calculute_products():
    ...
    simple_i = 0
    simple_product_d_vectorized = calculate_product_d_vectorized(simple_intervals[0][0], simple_intervals[0][1], simple_intervals, simple_mu, simple_sigma, simple_i)
    print('Simple product d vectorized:', simple_product_d_vectorized)
...
def system_of_equations(r_values, intervals, mu, sigma, n):
    # add the r_values to the intervals
    for i in range(len(intervals)):
        l_i, h_i = intervals[i]
        r_i = r_values[i]
        intervals[i] = (l_i + r_i, h_i + r_i)
    equations = []
    products = []
    first_product = calculate_product_d_vectorized(intervals[0][0], intervals[0][1], intervals, mu, sigma, 0)
    for i in range(1, len(intervals)):
        l_i, h_i = intervals[i]
        product = calculate_product_d_vectorized(l_i, h_i, intervals, mu, sigma, i)
