...
N = len(intervals)
r_values = []
initial_guess = [0.5, 0.5, 0.5] # try_something(intervals)
...
# Solve for r_values
# Define bounds for r_values (0 to 1)
bounds = [(0, 1) for _ in range(len(initial_guess))]
r_values = minimize(objective_function, initial_guess, args=(intervals, mean, std, N),method='trust-constr', bounds=bounds).x
