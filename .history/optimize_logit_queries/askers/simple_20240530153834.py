...
def some_over_n(low, high, constraints=None, real=None, error=None, **kwargs):
    ...
    w = high - low
    # i = np.argmax(w)
    # Find token with the smallest width that is not the base token
    w[base_top_token] = 1
    # Alternate between top token and smallest width token
...
def start_one_over_n_normal_V2(low, high, constraints=None, real=None, error=None, **kwargs):
    # just always make a guess slightly below the high value 
    r = 1 - (high - np.random.uniform(0.001, 0.01, size=len(high)))
    base_top_token, _ = constraints[0]
    r[base_top_token] = 0
    return r
