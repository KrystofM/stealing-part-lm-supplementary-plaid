...
def find_r_i(l_i, h_i, l_n, h_n, mu, sigma, n):
    ...
    for r in range(precision):
        ...
        diff = abs(calculate_product_d_uniform(l_i, h_i, r_i, l_n, h_n, mu, sigma) - (1/n)**(1/(n-1)))
        if diff < min_diff:
            min_diff = diff
            min_r_i = r_i
...
# Sample all at once
samples = np.array([sample_uniform(low[j], high[j], num_samples, mu, sigma) for j in range(n)])
print(samples.shape)
