...
def find_x_bigger_than_m_uniform_prior(l_i, h_i, n, mean, std):
    # Create the unirom
    dist = uniform(loc=0.5, scale=0.5)
    ...
    x = dist.isf(prob)
    # Original width
    original_width = h_i - l_i
    
    # Center of the original distribution
    original_center = (l_i + h_i) / 2
    x = original_center + (x - 0.5) * original_width

    return x

def sample_uniform(l_i, h_i, n, mean, std):
    dist = uniform(loc=(l_i + h_i) / 2, scale=(h_i - l_i))
    samples = dist.rvs(size=n)
    return samples
...
def find_x_bigger_than_m_for_intervals(intervals, mean, std):
    ...
    for i in range(n):
        l_i, h_i = intervals[i]
        x = find_x_bigger_than_m_uniform_prior(l_i, h_i, n, mean, std)
...
intervals = [(0.49, 0.5), (0.59999, 0.6), (0.79, 0.8), (0.39, 0.4)]
N = len(intervals)
r_values = []
...
for i in range(10000):
    ...
    for j in range(len(intervals)):
        sample = sample_uniform(intervals[j][0], intervals[j][1], 1, mean, std)
