...
def find_x_bigger_than_m(l_i, h_i, n, mean, std):
    ...
    # Adjust x by the width of the interval
    width = h_i - l_i
    center = (h_i + l_i) / 2
    adjusted_x = center + (x - center) / width
...
mean = -100  # assuming a mean, adjust as needed
std = 0.1   # assuming a standard deviation, adjust as needed
intervals = [(0.4, 0.5), (0.5, 0.6), (0.799, 0.8), (0.3, 0.4)]
N = len(intervals)
r_values = []
...
# Sample all at once
samples = np.array([sample_truncated_normal_adjusted(interval[0], interval[1], num_samples, mean, std) for interval in intervals])
