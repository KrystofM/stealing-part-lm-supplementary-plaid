...
pdf = truncnorm.pdf(x, (0 - mean) / std, (1 - mean) / std, loc=mean, scale=std)
plt.plot(x, pdf, label='Truncated Normal PDF')

# Highlight the intervals
for l_i, h_i in intervals:
    plt.fill_between(x, 0, pdf, where=(x >= l_i) & (x <= h_i), alpha=0.5)

plt.xlabel('x')
plt.ylabel('Probability Density')
plt.title('Truncated Normal Distribution with Intervals')
plt.legend()
plt.show()
...
for i in range(10000):
    ...
    for j in range(len(intervals)):
        sample = sample_truncated_normal_adjusted(intervals[j][0], intervals[j][1], 1, mean, std)
